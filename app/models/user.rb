class User < ActiveRecord::Base
  ...
  # Creates new remember token string and 
  # updates remember_digest with digest of given token.
  def remember
    self.remember_token = User.new_token
    update_attribute(:remember_digest, User.digest(remember_token))
  end

  # Returns true if given token match model's digest.
  def authenticated?(remember_token)
    return false if remember_digest.nil?
    BCrypt::Password.new(remember_digest).is_password?(remember_token)
  end

  # Deletes remember digest from model.
  def forget
    update_attribute(:remember_digest, nil)
  end

  # Calculates age from birth_date, or default.
  def age
    unless birth_date
      "N/A"
    else
